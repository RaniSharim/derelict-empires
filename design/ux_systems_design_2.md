...
All three surfaces **inherit the visual system** of `derelict_empires_ui_spec.md` without exception — glass material, faction color tokens, Exo 2 SemiBold (titles) / B612 Mono Bold (everything else) typography at Title/Normal/Small sizes (16/14/12), 40/36/44px click target minimums, no rounded corners, no drop shadows. No new colors, fonts, or materials are introduced here.
...
| Expertise visibility | **Any time a specific module is referenced, its current expertise multiplier appears next to it in `B612 Mono Bold` 12px**: `1.4×` (on-color bonus), `0.7×` (cross-color penalty). This is the same number shown in the Tech Tree overlay's Expertise Bar (§5.4 of research spec) — never computed differently across surfaces. |
...
- **Class + Variant**: `Exo 2 SemiBold 16px` ALL-CAPS. Class (`CRUISER`) in `TextBright`, variant (`Aggressive`) in `TextLabel`.
- **Slot summary**: `B612 Mono Bold 12px`. `3W 2D 1E 1S` means 3 weapon, 2 defense, 1 engine, 1 sensor. Universal slots shown as `U`.
- **Base stats**: 4 lines (HP, speed, visibility, supply base). Each: `B612 Mono Bold 12px` label in `TextDim`, `B612 Mono Bold 12px` value in `TextLabel`.
...
| Zone | Width | Content |
|---|---|---|
| Slot ID | 36px | `W1`, `D2`, `E1`, etc. — `B612 Mono Bold 12px` letter-spacing 1px, color `TextDim`. Faint to keep focus on the module name. |
...
| Module name | flex | Faction glow color if filled; `TextDim` if empty; dim faction base if locked. `Exo 2 SemiBold 16px` ALL-CAPS. |
| Expertise multiplier | 44px | `B612 Mono Bold 12px`, faction glow. `1.4×`, `0.7×`, etc. Only shown when filled. |
...
```
▮ RED          ← color subheader, Exo 2 SemiBold 16px, faction glow
...
1. **Stats Block** — HP, Armor (flat reduction), Shield HP + regen, Speed, Visibility (lower is better), Supply consumption (base). Each is a row: `B612 Mono Bold 12px` label / `B612 Mono Bold 14px` value / delta since last save (`+14`, `-22`) in `#66dd88` or `#ff6655` if applicable.
...
3. **Supply Cost Strip** — **the deep-trade pillar made visible.** One horizontal strip per color that the design consumes:
   ...
   Supply /s in combat
   ```
   - Strip height 3px, `B612 Mono Bold 12px` rate label.
...
| Element | Detail |
|---|---|
| Role name + count | `Exo 2 SemiBold 16px` ALL-CAPS, `TextBright` |
...
- Icon + label + `→` glyph
- `B612 Mono Bold 12px` weight-500 ALL-CAPS letter-spacing 1px
